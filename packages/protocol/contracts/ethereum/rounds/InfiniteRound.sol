...
contract InfiniteRound is IInfiniteRound, AssetRound {
    ...
    constructor(
        ...
        address _manager
    ) AssetRound(RoundType.INFINITE, _classHash, _propHouse, _starknet, _messenger, _roundFactory, _executionRelayer, _manager) {}
    ...
    function isAssetWinner(address user, uint256 proposalId, Asset[] calldata assets, IncrementalTreeProof calldata proof) public view returns (bool) {
        return IncrementalMerkleProof.verify(proof, MAX_WINNER_TREE_DEPTH, winnerMerkleRoot, _computeLeaf(proposalId, user, assets.packMany()));
    }
    ...
    /// @param assets The assets to claim
    /// @param proof The merkle proof used to verify the validity of the asset payout
    function _claimManyTo(address recipient, uint256 proposalId, Asset[] memory assets, IncrementalTreeProof memory proof) internal {
