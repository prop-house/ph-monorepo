...
contract MetadataEncoder is IMetadataEncoder {
    ...
    function encode(string calldata name, string calldata description, bytes calldata image) external pure returns (string memory) {
        // prettier-ignore
        return string.concat(
            'data:application/json;base64,',
            abi.encodePacked(
                '{', '"name": "', name, '",', '"description": "', description, '",', '"image": "data:image/svg+xml;base64,', image.encode(), '"' '}'
            ).encode()
        );
    }
    ...
    function encodeWithImageURL(string calldata name, string calldata description, bytes calldata imageURL) external pure returns (string memory) {
        // prettier-ignore
        return string.concat(
            'data:application/json;base64,',
            abi.encodePacked('{', '"name": "', name, '",', '"description": "', description, '",', '"image": "', imageURL, '"', '}').encode()
        );
    }
