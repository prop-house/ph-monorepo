...
contract PropHouse is IPropHouse, ERC721, AssetController {
    ...
    function createRoundOnExistingHouse(address house, Round calldata newRound) external payable returns (address round) {
        if (!isHouse(house)) {
    ...
    function createAndFundRoundOnExistingHouse(address house, Round calldata newRound, Asset[] calldata assets) external payable returns (address round) {
        if (!isHouse(house)) {
            revert INVALID_HOUSE();
        }
        if (!manager.isRoundRegistered(_getImpl(house), newRound.impl)) {
    ...
    /// @param newRound The round creation data
    function createRoundOnNewHouse(House calldata newHouse, Round calldata newRound) external payable returns (address house, address round) {
    ...
    /// @param newRound The round creation data
    /// @param assets Assets to deposit to the round
    function createAndFundRoundOnNewHouse(House calldata newHouse, Round calldata newRound, Asset[] calldata assets)
        external
        payable
