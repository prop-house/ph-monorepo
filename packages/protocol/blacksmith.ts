...
const createFunction = (name: string, fn: JsonFragment) => {
  ...
  const fmtArgs = (
    args: ReadonlyArray<JsonFragmentType>,
    withType: boolean,
    withName: boolean
  ) => {
  ...
  return `function ${
    fn.name
  }${fmtInput()} public ${fmtPayable()} startPrank stop ${fmtOutput()} {
        ${fmtReturn()}${name}(proxiedContract).${
          fn.name
        }${fmtValue()}${fmtInput(false)};
    }`;
};
...
const createCode = ({
  ...
  source,
  abi
}: SolidityContract & { abi: JsonFragment[] }) => {
