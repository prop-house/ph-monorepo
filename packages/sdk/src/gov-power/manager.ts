...
export class GovPowerManager<CS extends Custom | void = void> {
  ...
  constructor(private readonly _config: GovPowerChainConfig<CS>) {
    this._all = [
      ...this._defaults,
      ...(this._config.customStrategies || [])
    ].map(
      (Handler) =>
        new Handler(this._config) as StrategyHandlerBase<
          GovPowerStrategyConfig<CS>
        >
    );
  }
   ...
   */
  public static for<CS extends Custom | void = void>(
    config: GovPowerChainConfig<CS>
  ) {
  ...
  public get(typeOrAddress: string) {
    ...
    if (!strategy) {
      throw new Error(
        `Unknown governance power strategy type or address: ${typeOrAddress}`
      );
    }
    return strategy;
  }
   ...
   */
  public async getStrategyAddressAndParams(
    strategy: GovPowerStrategyConfig<CS>
  ) {
  ...
  public async getUserParamsForStrategies(
    ...
    return Promise.all(
      strategies.map(async (strategy) =>
        this.get(strategy.address).getUserParams(
          account,
          timestamp,
          strategy.id
        )
  ...
  public async getPreCallsForStrategies(
    ...
    const preCalls = await Promise.all(
      strategies.map(
        async (strategy) =>
          this.get(strategy.address).getStrategyPreCalls?.(
            account,
            timestamp,
            strategy.id
          )
  ...
  public async getTotalPower(
    ...
    const results = await this.getPowerForStrategies(
      user,
      timestamp,
      strategies
    );
    return results.reduce(
      (acc, curr) => acc.add(curr.govPower),
      BigNumber.from(0)
    );
  }
}
