...
export class AllowlistHandler extends StrategyHandlerBase<AllowlistConfig> {
  ...
  public async getStrategyParams(strategy: AllowlistConfig): Promise<string[]> {
    ...
    const { sortedValues, sortedMembers } = this.getTreeValues(
      strategy.members
    );
    ...
    const formattedIPFS = intsSequence.IntsSequence.LEFromString(IpfsHash);
  ...
  public async getUserParams(
    ...
    const { sortedMembers, sortedValues } = this.getTreeValues(
      allowlist.members
    );
    ...
    const { low, high } = splitUint256.SplitUint256.fromUint(
      BigInt(member.govPower)
    );
    return [
      member.address,
      low,
      high,
      ...merkle.MerklePedersen.getProof(sortedValues, memberIndex)
    ];
  }
  ...
  public async getPower(config: GovPowerConfig): Promise<BigNumber> {
    const cid = encoding.stringFromLE(
      config.params.slice(1).map((param) => param.toString())
    );
    const allowlist: AllowlistJson = await ipfs.getJSON(cid);
    const member = allowlist.members.find(
      (member) => member.address.toLowerCase() === config.user.toLowerCase()
    );
    if (!member) {
