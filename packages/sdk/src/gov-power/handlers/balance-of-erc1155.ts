...
export class BalanceOfERC1155Handler extends SingleSlotProofHandler<BalanceOfERC1155Config> {
  ...
  constructor(config: ChainConfig) {
    ...
    if (!this._traceRpcs[this._evmChainId]) {
      throw new Error(
        `No trace provider available for chain with ID: ${this._evmChainId}`
      );
    }
    this._traceProvider = new JsonRpcProvider(
      this._traceRpcs[this._evmChainId]
    );
  }
  ...
  public async getStrategyParams(
    strategy: BalanceOfERC1155Config
  ): Promise<string[]> {
  ...
  public async getUserParams(
    ...
    const proofInputs = await this.fetchProofInputs(
      contractAddress,
      slotKey,
      block
    );
  ...
  public async getPower(config: GovPowerConfig): Promise<BigNumber> {
    ...
    const balance = await this.contractFor(token).balanceOf(
      config.user,
      config.params[2],
      {
        blockTag: block
