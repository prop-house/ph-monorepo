...
export class CheckpointableERC721Handler extends SingleSlotProofHandler<CheckpointableERC721Config> {
   ...
   */
  public async getStrategyParams(
    strategy: CheckpointableERC721Config
  ): Promise<string[]> {
    if (
      strategy.address.toLowerCase() !==
      CheckpointableERC721Handler._NOUNS.ADDRESS
    ) {
      throw new Error(
        'This handler currently only supports the Nouns ERC721 token'
      );
    }
  ...
  public async getUserParams(
    ...
    const [contractAddress, numCheckpointsSlotIndex, checkpointsSlotIndex] =
      strategy.params;

    const numCheckpointsSlotKey = encoding.getSlotKey(
      account,
      numCheckpointsSlotIndex
    );
    const numCheckpointsSlotKeyU256 = splitUint256.SplitUint256.fromHex(
      numCheckpointsSlotKey
    );

    const block = await this.getBlockNumberForTimestamp(timestamp);
    const numCheckpoints = await this.contractFor(
      contractAddress
    ...
    const checkpointsSlotKey = encoding.getNestedSlotKey(
      [account, checkpointToQuery],
      checkpointsSlotIndex
    );
    const checkpointsSlotKeyU256 =
      splitUint256.SplitUint256.fromHex(checkpointsSlotKey);

    const [numCheckpointsProofInputs, checkpointsProofInputs] =
      await Promise.all([
  ...
  private contractFor(token: string) {
    return new Contract(
      token,
      [GET_CURRENT_VOTES_FUNC, NUM_CHECKPOINTS_FUNC],
      this._evm
    );
  }
}
