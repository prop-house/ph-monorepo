...
export class InfiniteRound<CS extends void | Custom = void> extends RoundBase<
  RoundType.INFINITE,
  CS
> {
  ...
  public static getState(
    _params: GetRoundStateParams<RoundType.INFINITE>
  ): Infinite.RoundState {
  ...
  public async getConfigStruct(
    config: Infinite.Config<CS>
  ): Promise<Infinite.ConfigStruct> {
    if (
      config.votePeriodDurationSecs < InfiniteRound.MIN_VOTE_PERIOD_DURATION
    ) {
  ...
  public encode(configStruct: Infinite.ConfigStruct): string {
    return defaultAbiCoder.encode(
      [InfiniteRound.CONFIG_STRUCT_TYPE],
      [configStruct]
    );
  }
  ...
  public async signProposeMessage(config: Infinite.ProposeConfig) {
    ...
    const message = {
      ...
      authStrategy: encoding.hexPadLeft(
        this._addresses.starknet.auth.infinite.sig
      ),
      requestedAssets: encoding
        .compressAssets(config.requestedAssets)
        .map(([assetId, amount]) => ({
          assetId,
          amount
        })),
    ...
    const signature = await this.signer._signTypedData(
      this.DOMAIN,
      this.pick(InfiniteRound.EIP_712_TYPES, [
        'Propose',
        'UserStrategy',
        'Asset'
      ]),
  ...
  public getVoteCalldata(config: Infinite.VoteCalldataConfig): string[] {
    return [
      ...
      ...config.proposalVotes.flatMap(
        ({ proposalId, proposalVersion, votingPower, direction }) => {
   ...
   */
  public async getSpentVotingPower(
    round: string,
    voter: string,
    proposalId: string,
