...
export class TimedRound<CS extends void | Custom = void> extends RoundBase<
  RoundType.TIMED,
  CS
> {
   ...
   */
  public static MIN_PROPOSAL_PERIOD_DURATION = Time.toSeconds(
    60,
    TimeUnit.Minutes
  );
   ...
   */
  public static for<CS extends void | Custom = void>(
    config: RoundChainConfig<CS>
  ) {
  ...
  public async getConfigStruct(
    config: Timed.Config<CS>
  ): Promise<Timed.ConfigStruct> {
    // prettier-ignore
    ...
    if (
      config.metaTx &&
      BigNumber.from(config.metaTx.deposit ?? 0).gt(0) &&
      !config.metaTx.relayer
    ) {
      throw new Error(
        'Must provide meta-transaction relayer when deposit is non-zero'
      );
    }
    ...
    if (
      config.awards.length !== 1 &&
      config.awards.length !== config.winnerCount
    ) {
      throw new Error(
    ...
    if (proposalThreshold.gt(0) && !config.proposingStrategies?.length) {
      throw new Error(
        'Round must have at least one proposing strategy when threshold is non-zero'
      );
    }
    ...
    const [proposingStrategies, votingStrategies] = await Promise.all([
      Promise.all(
        (config.proposingStrategies ?? []).map((s) =>
          this._govPower.getStrategyAddressAndParams(s)
        )
      ),
      Promise.all(
        config.votingStrategies.map((s) =>
          this._govPower.getStrategyAddressAndParams(s)
        )
      )
    ]);
    ...
    return {
      ...
      proposingStrategyParamsFlat: encoding.flatten2DArray(
        proposingStrategies.map((s) => s.params)
      ),
      votingStrategies: votingStrategies.map((s) => s.address),
      votingStrategyParamsFlat: encoding.flatten2DArray(
        votingStrategies.map((s) => s.params)
      ),
  ...
  public async getProposeEligibility(round: string, user: string) {
    ...
    const nonZeroStrategyProposingPowers =
      await this._govPower.getPowerForStrategies(
        user,
    ...
    return {
      canPropose: userProposingPower.gte(
        roundWithStrategies.config.proposalThreshold
      ),
  ...
  public async estimateMessageFee(configStruct: Timed.ConfigStruct) {
    const rawPayload = await this.getContract(this.impl).getRegistrationPayload(
      configStruct
    );
    const payload = [
    ...
    const response = await this._starknet['fetchEndpoint'](
  ...
  public async signProposeMessage(config: Timed.ProposeConfig) {
    ...
    const { proposingStrategiesRaw, config: timedConfig } =
      await this._query.getRound(config.round);
    ...
    if (
      timedConfig.proposalThreshold > 0 &&
      proposingStrategiesRaw.length > 0
    ) {
      const timestamp = await this.getProposingPeriodSnapshotTimestamp(
        config.round
      );
      nonZeroStrategyProposingPowers =
        await this._govPower.getPowerForStrategies(
          address,
          timestamp,
    ...
    const metadataUriIntsSequence = intsSequence.IntsSequence.LEFromString(
      config.metadataUri
    );
    const message = {
      ...
      proposer: address,
      authStrategy: encoding.hexPadLeft(
        this._addresses.starknet.auth.timed.sig
      ),
      usedProposingStrategies: nonZeroStrategyProposingPowers.map(
        ({ strategy }, i) => ({
          id: strategy.id,
  ...
  public async proposeViaSignature(config: Timed.ProposeConfig) {
    const { address, signature, message } = await this.signProposeMessage(
      config
    );
  ...
  public async editProposalViaSignature(config: Timed.EditProposalConfig) {
    const { address, signature, message } = await this.signEditProposalMessage(
      config
    );
    return this.sendToRelayer<Timed.RequestParams>({
      address,
  ...
  public async cancelProposalViaSignature(config: Timed.CancelProposalConfig) {
    const { address, signature, message } =
      await this.signCancelProposalMessage(config);
    return this.sendToRelayer<Timed.RequestParams>({
      address,
      signature,
  ...
  public async signVoteMessage(config: Timed.VoteConfig) {
    ...
    const { govPowerStrategiesRaw } =
      await this._query.getRoundVotingStrategies(config.round);
    ...
    const nonZeroStrategyVotingPowers =
      await this._govPower.getPowerForStrategies(
        address,
        timestamp,
    ...
    const message = {
      ...
      usedVotingStrategies: nonZeroStrategyVotingPowers.map(
        ({ strategy }, i) => ({
          id: strategy.id,
          userParams: userParams[i]
        })
    ...
    const signature = await this.signer._signTypedData(
      this.DOMAIN,
      this.pick(TimedRound.EIP_712_TYPES, [
        'Vote',
        'ProposalVote',
        'UserStrategy'
      ]),
  ...
  public async relaySignedProposePayload(
    ...
    const call = this.createEVMSigAuthCall(
      payload,
      'authenticate_propose',
      this.getProposeCalldata(params.data)
    );
  ...
  public async relaySignedEditProposalPayload(
    ...
    const call = this.createEVMSigAuthCall(
      payload,
      'authenticate_edit_proposal',
      this.getEditProposalCalldata(params.data)
    );
  ...
  public async relaySignedCancelProposalPayload(
    ...
    const call = this.createEVMSigAuthCall(
      payload,
      'authenticate_cancel_proposal',
      this.getCancelProposalCalldata(params.data)
    );
    const fee = await account.estimateFee(call);
    return account.execute(call, undefined, {
      maxFee: fee.suggestedMaxFee
  ...
  public async relaySignedVotePayload(
    ...
    const timestamp = await this.getVotingPeriodSnapshotTimestamp(
      params.data.round
    );
    const { govPowerStrategiesRaw } = await this._query.getGovPowerStrategies({
      where: {
    ...
    const call = this.createEVMSigAuthCall(
      payload,
      'authenticate_vote',
      this.getVoteCalldata(params.data)
    );
    const fee = await account.estimateFee([...preCalls, call], {
   ...
   */
  public async determineWinners(
    account: Account,
    config: Timed.FinalizationConfig
  ) {
  ...
  public async cancel(roundAddress: string) {
    return this.getContract(roundAddress)
      .connect(this.signer)
      .cancel({ value: 2e14 });
  }
  ...
  public async claimAward(config: Timed.ClaimAwardConfig) {
    const [round, winningProposals] = await Promise.all([
      ...
      this._query.getProposalsForRound(config.round, {
        where: {
          isWinner: true
        },
    ...
    if (!winningProposals.length)
      throw new Error(`No winning proposals for round ${config.round}`);
    ...
    const { proposal, leaf, assetStruct } =
      proposalLeaves.find(
        ({ proposal }) => proposal.id === config.proposalId
      ) || {};
    if (!proposal || !leaf || !assetStruct)
      throw new Error(
        `Proposal, leaf, or asset not found for proposal ID ${config.proposalId}`
      );

    const tree = this.generateClaimMerkleTree(
      proposalLeaves.map(({ leaf }) => leaf)
    );
  ...
  public getProposeCalldata(config: Timed.ProposeCalldataConfig): string[] {
    const metadataUri = intsSequence.IntsSequence.LEFromString(
      config.metadataUri
    );
    return [
      config.proposer,
      ...
      ...config.usedProposingStrategies
        .flatMap(({ id, userParams }) => [id, userParams.length, ...userParams])
        .map((v) => v.toString())
    ];
  }
   ...
   */
  public getEditProposalCalldata(
    config: Timed.EditProposalCalldataConfig
  ): string[] {
    const metadataUri = intsSequence.IntsSequence.LEFromString(
      config.metadataUri
    );
   ...
   */
  public getCancelProposalCalldata(
    config: Timed.CancelProposalCalldataConfig
  ): string[] {
  ...
  public getVoteCalldata(config: Timed.VoteCalldataConfig): string[] {
    return [
      ...
      ...config.proposalVotes.flatMap(({ proposalId, votingPower }) => {
        const p = splitUint256.SplitUint256.fromUint(
   ...
   */
  public async getProposingPeriodSnapshotTimestamp(
    round: string
  ): Promise<string> {
   ...
   * @param round The Starknet round address
   */
  public async getVotingPeriodSnapshotTimestamp(
    round: string
  ): Promise<string> {
    const config = await this._starknet.getStorageAt(
      round,
  ...
  public async getSpentVotingPower(round: string, voter: string) {
    const key = encoding.getStorageVarAddress(
      this._SPENT_VOTING_POWER_STORE,
      voter
    );
   ...
   */
  protected generateClaimMerkleTree(
    winnersOrLeaves: Timed.RoundWinner[] | string[]
  ) {
