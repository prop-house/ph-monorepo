...
export class PropHouse<CS extends Custom | void = void> extends ChainBase {
  ...
  public async batchDepositTo(
    ...
    if (assets.length > 1) {
      const { assets: _assets, value } =
        this.mergeAssetsAndGetTotalETHValue(assets);
  ...
  public async createRoundOnExistingHouse<RT extends RoundType>(
    ...
    const starknetFeeEstimate = await this.round.estimateMessageFee(
      round.roundType,
      struct
    );
    const cappedStarknetFee = this.min(
      starknetFeeEstimate,
      PropHouse._MAX_STARKNET_MESSAGE_FEE_CAP
    );
    ...
    const inputs = [
      houseAddress,
      {
        ...
        config: this.round.encode(
          round.roundType,
          struct as RoundConfigStruct[RT]
        )
      }
    ] as const;
    ...
    const gasLimit = await this.contract.estimateGas.createRoundOnExistingHouse(
      ...inputs,
      overridesWithValue
    );
  ...
  public async createAndFundRoundOnExistingHouse<RT extends RoundType>(
    ...
    const overridesWithValue = {
      ...
      value: BigNumber.from(value)
        .add(cappedStarknetFee)
        .add(struct.metaTx.deposit)
    };
    const gasLimit =
      await this.contract.estimateGas.createAndFundRoundOnExistingHouse(
        ...inputs,
        overridesWithValue
      );
   ...
   * @param overrides Optional transaction overrides
   */
  public async createRoundOnNewHouse<
    HT extends HouseType,
    RT extends RoundType
  >(house: HouseInfo<HT>, round: RoundInfo<RT, CS>, overrides: Overrides = {}) {
    const struct = await this.round.getConfigStruct(round);
    ...
    const gasLimit = await this.contract.estimateGas.createRoundOnNewHouse(
      ...inputs,
      overridesWithValue
    );
   ...
   * @param overrides Optional transaction overrides
   */
  public async createAndFundRoundOnNewHouse<
    HT extends HouseType,
    RT extends RoundType
  >(
    house: HouseInfo<HT>,
    ...
    const gasLimit =
      await this.contract.estimateGas.createAndFundRoundOnNewHouse(
        ...inputs,
        overridesWithValue
      );
