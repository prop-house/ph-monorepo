...
export class QueryWrapper {
  ...
  constructor(chainId: number) {
    this._gql = getGraphQlClientsForChainOrThrow(
      chainId,
      (url) => new GraphQLClient(url)
    );
  }
  ...
  public async getHousesWhereAccountIsOwnerOrHasCreatorPermissions(
    ...
    return this.getHouses({
      ...
      where: {
        ...config.where,
        or: [
          { roundCreators_: { creator: ownerOrCreator } },
  ...
  public async getHouse(houseAddress: Address): Promise<House> {
    const { house } = await this._gql.evm.request(HouseQuery, {
      id: houseAddress.toLowerCase()
    });
    if (!house) {
  ...
  public async getRoundsRelevantToAccount(
    ...
    const { proposals, votes } = await this._gql.starknet.request(
      ManyRoundsWhereProposerOrVoterQuery,
      {
    ...
    const proposalRounds =
      proposals?.map((p) => p!.round.sourceChainRound) ?? [];
  ...
  public async getRound(roundAddress: Address): Promise<Round> {
    const { round } = await this._gql.evm.request(RoundQuery, {
  ...
  public async getRoundVoteCount(roundAddress: Address): Promise<string> {
    ...
    return (
      votes?.reduce((acc, v) => acc + BigInt(v!.votingPower), BigInt(0)) ??
      BigInt(0)
    ).toString();
  }
  ...
  public async getRoundProposalCount(roundAddress: Address): Promise<number> {
    const { rounds } = await this._gql.starknet.request(
      RoundProposalCountQuery,
      {
        sourceChainRound: roundAddress.toLowerCase()
      }
   ...
   * @param roundAddress The round address
   */
  public async getRoundMerkleRoot(
   ...
   * @param roundAddress The round address
   */
  public async getRoundWithHouseInfo(
    roundAddress: Address
  ): Promise<RoundWithHouse> {
  ...
  ): Promise<RoundBalance[]> {
    const { balances } = await this._gql.evm.request(
      ManyBalancesQuery,
      toPaginated(
        this.merge(getDefaultConfig(Balance_OrderBy.UpdatedAt), config)
      )
    );
  ...
  public async getGovPowerStrategies(
    config: Partial<
      QueryConfig<GovPowerStrategy_OrderBy, GovPowerStrategy_Filter>
    > = {}
  ) {
    const { govPowerStrategies: govPowerStrategiesRaw } =
      await this._gql.evm.request(
        ManyGovPowerStrategiesQuery,
        toPaginated(
          this.merge(getDefaultConfig(GovPowerStrategy_OrderBy.Id), config)
        )
      );
    return {
      govPowerStrategiesRaw,
      govPowerStrategies: await Promise.all(
        govPowerStrategiesRaw.map((strategy) =>
          this.toParsedGovPowerStrategy(strategy)
        )
      )
    };
  }
  ...
  public async getDeposits(
    ...
    const { deposits } = await this._gql.evm.request(
      ManyDepositsQuery,
      toPaginated(
        this.merge(getDefaultConfig(Deposit_OrderBy.DepositedAt), config)
      )
    );
   ...
   * @param config Filtering, pagination, and ordering configuration
   */
  public async getReclaims(
    config: Partial<QueryConfig<Reclaim_OrderBy, Reclaim_Filter>> = {}
  ): Promise<Reclaim[]> {
    const { reclaims } = await this._gql.evm.request(
      ManyReclaimsQuery,
      toPaginated(
        this.merge(getDefaultConfig(Reclaim_OrderBy.ReclaimedAt), config)
      )
    );
  ...
  public async getProposals(
    ...
    const { proposals } = await this._gql.starknet.request(
      ManyProposalsQuery,
      toPaginated(
        this.merge(getDefaultConfig(OrderByProposalFields.ReceivedAt), config)
      )
    );
  ...
  public async getProposal(
    ...
    if (!proposals?.length) {
      throw new Error(
        `Proposal ID ${proposalId} not found for round ${roundAddress}`
      );
    }
    return proposals[0];
  }
  ...
  public async getVotes(
    ...
    const { votes } = await this._gql.starknet.request(
      ManyVotesQuery,
      toPaginated(
        this.merge(getDefaultConfig(OrderByVoteFields.ReceivedAt), config)
      )
    );
    return votes!.map((v) => ({
      voter: v!.voter.id,
  ...
  protected toHouse(house: IHouseQuery['house']): House {
    if (!house)
      throw new Error(
        'House information not present during attempted conversion'
      );
    return {
      address: house.id,
  ...
  protected async toRound(round: IRoundQuery['round']): Promise<Round> {
    if (!round)
      throw new Error(
        'Round information not present during attempted conversion'
      );
    if (!round.timedConfig)
      throw new Error(
        'Round config information not present during attempted conversion'
      );

    const config = round.timedConfig;
    const proposingStrategiesRaw = round.proposingStrategies.map(
      ({ strategy }) => strategy
    );
    const votingStrategiesRaw = round.votingStrategies.map(
      ({ strategy }) => strategy
    );
    return {
      address: round.id,
      ...
      votingStrategiesRaw,
      proposingStrategies: await Promise.all(
        proposingStrategiesRaw.map((strategy) =>
          this.toParsedGovPowerStrategy(strategy)
        )
      ),
      votingStrategies: await Promise.all(
        votingStrategiesRaw.map((strategy) =>
          this.toParsedGovPowerStrategy(strategy)
        )
      )
    };
    ...
    switch (award.asset.assetType) {
      case 'NATIVE':
        return {
          ...
          amount:
            (award as RawRoundAward).amount ??
            (award as RawRoundBalance).balance
        };
      case 'ERC20':
        return {
  ...
  protected async toParsedGovPowerStrategy(strategy: {
    ...
    switch (strategy.type) {
      ...
      case GovPowerStrategyType.ALLOWLIST: {
        const cid = encoding.stringFromLE(
          strategy.params.slice(1).map((p) => p.toString())
        );
  ...
  protected toProposal(proposal: IProposalQuery['proposal']): Proposal {
    if (!proposal)
      throw new Error(
        'Proposal information not present during attempted conversion'
      );
    return {
   ...
   * @param round The round to convert
   */
  protected async toRoundWithHouseInfo(
    round: IRoundWithHouseInfoQuery['round']
  ): Promise<RoundWithHouse> {
    if (!round)
      throw new Error(
        'Round information not present during attempted conversion'
